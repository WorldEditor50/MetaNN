...
namespace MetaNN
{
namespace OperSquare::NSCaseGen
{
...
template <>
struct OperSeq_<OpTags::Square>
{
    using type = OperCalAlgoChain<TailCalculator<OperSquare::NSCaseGen::EvalItem, OperSquare::NSCaseGen::EvalGroup>>;
};

template <typename TP,
          std::enable_if_t<IsValidOper<OpTags::Square, TP>>* = nullptr>
auto Square(TP&& p_m)
{
    using rawM = RemConstRef<TP>;
    using ResType = Operation<OpTags::Square, OperandContainer<rawM>>;
...
namespace MetaNN
{
namespace OperSquareGrad::NSCaseGen
{
    ...
    protected:
        virtual void EvalInternalLogic(EvalItemType& evalItem) final override
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                mem_out[i] = 2 * mem_grad[i % grad_count] * mem_in[i];
            }
            evalItem.m_outputHandle.SetData(std::move(out));
        }
    };
...
template <>
struct OperSeq_<OpTags::SquareGrad>
{
    using type = OperCalAlgoChain<TailCalculator<OperSquareGrad::NSCaseGen::EvalItem, OperSquareGrad::NSCaseGen::EvalGroup>>;
};

template <typename TGrad, typename TInput,
          std::enable_if_t<IsValidOper<OpTags::SquareGrad, TGrad, TInput>>* = nullptr>
auto SquareGrad(TGrad&& p_grad, TInput&& p_input)
{
