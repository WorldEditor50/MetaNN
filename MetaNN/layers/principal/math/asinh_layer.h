...
namespace MetaNN
{
    ...
    public:
        ...
        auto FeedForward(TIn&& p_in)
        {
            ...
            return LayerOutputCont<AsinhLayer>().template Set<LayerOutput>(std::move(res));
        }
        ...
        auto FeedBackward(TGrad&& p_grad)
        {
            if constexpr (!IsFeedbackOutput || RemConstRef<TGrad>::template IsValueEmpty<LayerOutput>)
            {
                ...
                return LayerInputCont<AsinhLayer>();
            }
            else
            {
                if (m_data.empty())
                {
                    throw std::runtime_error("Cannot feed back in AsinhLayer");
                }
                auto input = m_data.top();

                auto grad = std::forward<TGrad>(p_grad).template Get<LayerOutput>();
                auto res = AsinhGrad(std::move(grad), std::move(input));
                m_inputShape.CheckDataShape(res);
                
                LayerTraits::PopoutFromStack(m_data, m_inputShape);
                return LayerInputCont<AsinhLayer>().template Set<LayerInput>(std::move(res));
            }
        }
