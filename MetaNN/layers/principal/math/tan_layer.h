...
namespace MetaNN
{
    ...
    public:
        ...
        auto FeedForward(TIn&& p_in)
        {
            ...
            return LayerOutputCont<TanLayer>().template Set<LayerOutput>(std::move(res));
        }
        ...
        auto FeedBackward(TGrad&& p_grad)
        {
            if constexpr (!IsFeedbackOutput || RemConstRef<TGrad>::template IsValueEmpty<LayerOutput>)
            {
                ...
                return LayerInputCont<TanLayer>();
            }
            else
            {
                if (m_data.empty())
                {
                    throw std::runtime_error("Cannot feed back in TanLayer");
                }
                auto input = m_data.top();

                auto grad = std::forward<TGrad>(p_grad).template Get<LayerOutput>();
                auto res = TanGrad(std::move(grad), std::move(input));
                m_inputShape.CheckDataShape(res);
                
                LayerTraits::PopoutFromStack(m_data, m_inputShape);
                return LayerInputCont<TanLayer>().template Set<LayerInput>(std::move(res));
            }
        }
