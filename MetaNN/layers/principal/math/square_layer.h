...
namespace MetaNN
{
    ...
    public:
        ...
        auto FeedForward(TIn&& p_in)
        {
            ...
            return LayerOutputCont<SquareLayer>().template Set<LayerOutput>(std::move(res));
        }
        ...
        auto FeedBackward(TGrad&& p_grad)
        {
            if constexpr (!IsFeedbackOutput || RemConstRef<TGrad>::template IsValueEmpty<LayerOutput>)
            {
                ...
                return LayerInputCont<SquareLayer>();
            }
            else
            {
                if (m_data.empty())
                {
                    throw std::runtime_error("Cannot feed back in SquareLayer");
                }
                auto input = m_data.top();

                auto grad = std::forward<TGrad>(p_grad).template Get<LayerOutput>();
                auto res = SquareGrad(std::move(grad), std::move(input));
                m_inputShape.CheckDataShape(res);
                
                LayerTraits::PopoutFromStack(m_data, m_inputShape);
                return LayerInputCont<SquareLayer>().template Set<LayerInput>(std::move(res));
            }
        }
