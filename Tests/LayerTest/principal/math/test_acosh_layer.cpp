...
namespace
{
    ...
    void test_acosh_layer1()
    {
        cout << "Test acosh layer case 1 ...\t";
        using RootLayer = MakeInferLayer<AcoshLayer>;
        ...
        assert(fabs(res(0, 0) - std::acosh(1.27f)) < 0.001);
        assert(fabs(res(1, 0) - std::acosh(1.41f)) < 0.001);
    ...
    void test_acosh_layer2()
    {
        cout << "Test acosh layer case 2 ...\t";
        using RootLayer = MakeTrainLayer<AcoshLayer, CommonInputMap, PFeedbackOutput>;
        static_assert(RootLayer::IsFeedbackOutput);
        static_assert(!RootLayer::IsUpdate);
    ...
    void test_acosh_layer3()
    {
        cout << "Test acosh layer case 3 ...\t";
        using RootLayer = MakeTrainLayer<AcoshLayer, CommonInputMap, PFeedbackOutput>;
        static_assert(RootLayer::IsFeedbackOutput);
        static_assert(!RootLayer::IsUpdate);
        ...
        for (size_t loop_count = 1; loop_count < 10; ++loop_count)
        {
            ...
            for (size_t i = 0; i < loop_count; ++i)
            {
                ...
                {
                    assert(fabs(res(i, j) - std::acosh(in(i, j))) < 0.0001);
                }
            }
        }
    ...
    void test_acosh_layer4()
    {
        cout << "Test acosh layer case 4 (dummy grad input)...\t";
        using RootLayer = MakeTrainLayer<AcoshLayer, CommonInputMap, PFeedbackOutput>;
        static_assert(RootLayer::IsFeedbackOutput);
        static_assert(!RootLayer::IsUpdate);
