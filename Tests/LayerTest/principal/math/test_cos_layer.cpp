...
namespace
{
    ...
    void test_cos_layer1()
    {
        cout << "Test cos layer case 1 ...\t";
        using RootLayer = MakeInferLayer<CosLayer>;
        ...
        assert(fabs(res(0, 0) - std::cos(-0.27f)) < 0.001);
        assert(fabs(res(1, 0) - std::cos(-0.41f)) < 0.001);
    ...
    void test_cos_layer2()
    {
        cout << "Test cos layer case 2 ...\t";
        using RootLayer = MakeTrainLayer<CosLayer, CommonInputMap, PFeedbackOutput>;
        static_assert(RootLayer::IsFeedbackOutput);
        static_assert(!RootLayer::IsUpdate);
        ...
        assert(fabs(fb(0, 0) + 0.1f * std::sin(-0.27f)) < 0.001);
        assert(fabs(fb(1, 0) + 0.3f * std::sin(-0.41f)) < 0.001);

        LayerNeutralInvariant(layer);
        cout << "done" << endl;
    }

    void test_cos_layer3()
    {
        cout << "Test cos layer case 3 ...\t";
        using RootLayer = MakeTrainLayer<CosLayer, CommonInputMap, PFeedbackOutput>;
        static_assert(RootLayer::IsFeedbackOutput);
        static_assert(!RootLayer::IsUpdate);
        ...
        for (size_t loop_count = 1; loop_count < 10; ++loop_count)
        {
            ...
            for (size_t i = 0; i < loop_count; ++i)
            {
                ...
                {
                    assert(fabs(res(i, j) - std::cos(in(i, j))) < 0.0001);
                }
            }
        }
        ...
        for (size_t loop_count = 9; loop_count >= 1; --loop_count)
        {
            ...
            for (size_t i = 0; i < loop_count; ++i)
            {
                for (size_t j = 0; j < 3; ++j)
                {
                    auto aim = -grad(i, j) * std::sin(in(i, j));
    ...
    void test_cos_layer4()
    {
        cout << "Test cos layer case 4 (dummy grad input)...\t";
        using RootLayer = MakeTrainLayer<CosLayer, CommonInputMap, PFeedbackOutput>;
        static_assert(RootLayer::IsFeedbackOutput);
        static_assert(!RootLayer::IsUpdate);
