...
namespace
{
    ...
    void test_tan_grad_case3()
    {
        ...
        for (size_t p = 0; p < 2; ++p)
        {
            for (size_t i = 0; i < 10; ++i)
            {
                for (size_t k = 0; k < 7; ++k)
                {
                    auto check = grad(p, i, k) / std::cos(ori(p, i, k)) / std::cos(ori(p, i, k));
    ...
    void test_tan_grad_case4()
    {
        ...
        for (size_t i = 0; i < 10; ++i)
        {
            auto check = grad[i].Value() / std::cos(ori[i].Value()) / std::cos(ori[i].Value());
    ...
    void test_tan_grad_case5()
    {
        ...
        for (size_t p = 0; p < 2; ++p)
        {
            for (size_t i = 0; i < 10; ++i)
            {
                for (size_t k = 0; k < 7; ++k)
                {
                    auto check = grad(i, k) / std::cos(ori(p, i, k)) / std::cos(ori(p, i, k));
                    assert(fabs(res(p, i, k) - check) < 0.001f);
                }
            }
        }
